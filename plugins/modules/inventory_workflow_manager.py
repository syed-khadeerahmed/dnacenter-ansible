...
class Inventory(DnacBase):
    ...
    def get_diff_merged(self, config):
        ...
        if config.get('role'):
            self.log("inside")
            devices_to_update_role = self.get_device_ips_from_config_priority()
            self.log(devices_to_update_role)
            device_role = config.get('role')
            # for device in devices_to_update_role:
            #     device_role = None
            #     for config_entry in self.config:
            #         if device in config_entry.get('ip_address_list', []):
            #             device_role = config_entry.get('role')
            #             break
